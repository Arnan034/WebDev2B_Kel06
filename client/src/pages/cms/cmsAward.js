...
const CMSAward = () => {
  ...
  const fetchAwards = async () => {
    setLoading(true);
    try {
      const response = await apiServiceAdmin.getAllAward();
  ...
  const handleSubmit = async () => {
    try {
      const response = await apiServiceAdmin.createAward({
      ...
      setMessage(`Award "${response.data.data.institution}", tahun "${response.data.data.year}", kategori "${response.data.data.name}" berhasil ditambahkan!`);
  ...
  const handleEdit = (id, institution, year, name) => {
    setEditingId(id);
    setEditedAwards({ institution: institution, year: year, name: name });
  };
  ...
  const handleUpdate = async () => {
    try {
      await apiServiceAdmin.updateAward(editingId, editedAwards);
      setEditingId(null);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await apiServiceAdmin.deleteAward(id);
            ...
            {currentItems.map((award, index) => (
              <tr key={award.id_award}>
                ...
                <td className="text-center actions-column">
                  {editingId === award.id_award ? (
                    <>
                      <Button color='green' size='tiny' onClick={() => handleUpdate()}>Save</Button>
