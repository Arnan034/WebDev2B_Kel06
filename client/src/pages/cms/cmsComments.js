...
const CMSComments = () => {
  ...
  const fetchCommentData = useCallback(async () => {
    setLoading(true);
    try {
      const response = await apiServiceAdmin.getAllComment({
        filter: filter
      });
  ...
  const handleApproveSelected = async () => {
    try {
      const response = await apiServiceAdmin.updateApproveComment({
  ...
  const handleDeleteSelected = async () => {
    try {
      ...
      const response = await apiServiceAdmin.deleteComment({
        ids: selectedComments,
      });
      fetchCommentData();
      setSuccessMessage(response.data.message);
