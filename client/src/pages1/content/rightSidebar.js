...
const RightSidebar = ({ isSidebarOpen, onFiltersChange, handleSortChange }) => {
    ...
    useEffect(() => {
        const fetchAward = async () => {
            try {
                const response = await axios.get('http://localhost:5000/api/awards/institution');
                const options = response.data.map(award => ({ value: award.institution, label: award.institution }));
