...
const Comment = ({ id }) => {
    ...
    const fetchComment = useCallback(async () => {
        setLoading(true);
        try {
            const response = await axios.get(`http://localhost:5000/api/comments/byFilm/${id}`, { params: { filter } });
    ...
    const renderStars = (rating) => {
        const stars = [];
        for (let i = 1; i <= 5; i++) {
            stars.push(i <= rating ? '★' : '☆'); // Display solid or empty stars based on rating
        }
        return stars.join('');
    };
