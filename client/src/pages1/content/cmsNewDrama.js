...
const NewDrama = () => {
  ...
  useEffect(() => {
    const fetchFilmData = async () => {
      if (url_film.current) {
        try {
          const response = await axios.get(`http://localhost:5000/api/film/get-edit/${url_film.current}`);
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/film', formDataToSubmit, {
        headers: {
  ...
  const saveEdit = async () => {
    ...
    try {
        const response = await axios.put(`http://localhost:5000/api/film/save-edit/${id_film}`, formDataToSubmit, {
            headers: {
                'Content-Type': 'multipart/form-data'
            }
        });
